...
function specificGibbsEnergy_TXp_derT
  ...
  output Real gderT( unit="J/(kg.K)") "Derivative of temperature of specific gibbs energy";
protected
...
algorithm
  gderT:= 1000*(B*X + 2*T*C*X + 3*(T^2)*D*X + 4*(T^3)*E*{1,X[2]} -F*{1,X[2]}/(T-T0)^2
  + pkPa*(V[4] + V[5]*X[2] + V[6]*X[3] + 2*V[7]*T + 2*V[8]*X[2]*T)
  + (1/T)*L*X + (log(T)+1)*M*X);
  annotation(smoothOrder = 10, derivative=Absolut.Media.LiBrH2O.specificGibbsEnergy_TXp_derT_der);
end specificGibbsEnergy_TXp_derT;
