...
function specificGibbsEnergy_TXp_derT_derp
  ...
  output Real gderTderp "Derivative of pressure of derivative of temperature of specific gibbs energy";
protected
  ...
  // sometimes got problems with these implementation... prefer to use the one above (directly write values)
  Real[5] c_ = {0,1,2,3,1.1} "Exponent coef. for A, B and C";
  Real[5] X__ = {(X_LiBr*100)^c_[i] for i in 1:5} "LiBr mass fraction in [%] = {1,X_LiBr,X_LiBr^2, ...}";

algorithm
  gderTderp:= 1000*((V[4] + V[5]*X_[2] + V[6]*X_[3] + 2*V[7]*T + 2*V[8]*X_[2]*T));
  annotation(smoothOrder = 10);
end specificGibbsEnergy_TXp_derT_derp;
