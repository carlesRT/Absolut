...
function specificGibbsEnergy_TXp_derT_der_check
...
protected
  ...
  Real[5] X_ = {(X_LiBr*100)^c_[i] for i in 1:5} "LiBr mass fraction in [%] = {1,X_LiBr,X_LiBr^2, ...}";

algorithm
  der_gderT:= (specificGibbsEnergy_TXp_derT_derX(T,X_H2O,p))*(der_X_LiBr*100)
  + (specificGibbsEnergy_TXp_derT_derT(T,X_H2O,p))*der_T
  + (specificGibbsEnergy_TXp_derT_derp(T,X_H2O,p))*der_pkPa;

end specificGibbsEnergy_TXp_derT_der_check;
