...
function specificGibbsEnergy_TXp_derT_derT
  "Derivate of pecificGibbsEnergy respect to Temperature of the H2O/LiBr solution as a function of T, X_H2O and p"
  ...
  output Real gderT2( unit="J/(kg.K2)") "twice derivative of temperature of specific gibbs energy";
protected
  ...
  Real[5] X_ = {(X_LiBr*100)^c[i] for i in 1:5} "LiBr mass fraction in [%] = {1,X_LiBr,X_LiBr^2, ...}";
  Real[5] X = {1,(X_LiBr*100),(X_LiBr*100)^2,(X_LiBr*100)^3,(X_LiBr*100)^1.1} "LiBr mass fraction in [%] = {1,X_LiBr,X_LiBr^2, ...}";

algorithm
  gderT2:= 1000*(2*C*X + 6*T*D*X + 12*E*{1,X[2]}*T^2 + 2*F*{1,X[2]}/(T-T0)^3
  + pkPa*(2*V[7] + 2*V[8]*X[2]) - (1/T^2)*L*X + (1/T)*M*X);
  annotation(smoothOrder = 10);
end specificGibbsEnergy_TXp_derT_derT;
