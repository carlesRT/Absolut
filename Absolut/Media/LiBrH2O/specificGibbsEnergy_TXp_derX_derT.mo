...
function specificGibbsEnergy_TXp_derX_derT
  ...
  output Real gderXderT "Derivative respect to X_H2O of the specific gibbs energy";
protected
...
algorithm
  gderXderT:= 1000*(B*X + 2*T*C*X + 3*D*X*T^2 + 4*E*{0,1}*T^3 - F*{0,1}/(T-T0)^2
  + pkPa*(V[5] + V[6]*X[3] + 2*V[8]*T)
  + (1/T)*L*X + (log(T)+1)*M*X);
  annotation(smoothOrder = 10);
end specificGibbsEnergy_TXp_derX_derT;
