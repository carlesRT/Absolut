...
function specificHeatCapacity_SSC_TXp
...
protected
  ...
  //Real[5] X = {(X_LiBr*100)^c[i] for i in 1:5} "LiBr mass fraction in [%] = {1, X_LiBr, X_LiBr^2, X_LiBr^3, X_LiBr^1,1}";
  Real[5] X = {1,(X_LiBr*100),(X_LiBr*100)^2,(X_LiBr*100)^3,(X_LiBr*100)^1.1}  "LiBr mass fraction in [%] = {1, X_LiBr, X_LiBr^2, X_LiBr^3, X_LiBr^1,1}";

algorithm
  cp := 1000*(-2*T*C*X - 6*(T^2)*D*X - 12*(T^3)*E*{1,X[2]} - 2*T*F*{1,X[2]}/(T-T0)^3 - 2*pkPa*T*{V[7],V[8]}*{1,X[2]} + (1/T)*L*X - M*X);
  annotation(Inline=true,smoothOrder = 10, derivative=Absolut.Media.LiBrH2O.specificHeatCapacity_SSC_TXp_der);
end specificHeatCapacity_SSC_TXp;
