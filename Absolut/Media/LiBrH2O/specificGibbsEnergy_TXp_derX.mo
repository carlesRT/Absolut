...
function specificGibbsEnergy_TXp_derX
  ...
  output Real gderX "Derivative respect to X_H2O of the specific gibbs energy";
protected
  ...
  Real[5] c = {0,0,1,2,0.1} "Exponent coef. for A, B and C";
  Real[5] b = {0,1,2,3,1.1} "Exponent coef. for A, B and C";
  // Using b[i] .... c[i] yields wierd results c[1] has influence on the results! even if multiplied by 0...
  //Real[5] X = {b[i]*(X_LiBr*100)^c[i] for i in 1:5} "LiBr mass fraction in [%] = {0,1,2*X_LiBr,3*X_liBr^2, 1,1*X_LiBr^0,1}";
  Real[5] X = {0,1,2*X_LiBr*100,3*(X_LiBr*100)^2, 1.1*(X_LiBr*100)^0.1} "LiBr mass fraction in [%] = {0,1,2*X_LiBr,3*X_liBr^2, 1,1*X_LiBr^0,1}";

algorithm
  gderX:= 1000*(A*X + B*X*T + C*X*T^2 + D*X*T^3 + E*{0,1}*T^4 + F*{0,1}/(T-T0)
  + pkPa*(V[2] + V[3]*X[3] + V[5]*T + V[6]*X[3]*T + V[8]*T^2)
  + log(T)*L*X + T*log(T)*M*X);
annotation(smoothOrder = 10);
end specificGibbsEnergy_TXp_derX;
