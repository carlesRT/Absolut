...
function specificGibbsEnergy_TXp_derT_derX
  ...
  output Real gderTderX "Derivative of Mass Fraction of Derivative of temperature of specific gibbs energy";
protected
...
algorithm
  gderTderX:= 1000*(B*X + 2*T*C*X + 3*(T^2)*D*X + 4*E[2]*T^3 -F[2]/(T-T0)^2
  + pkPa*(V[5] + V[6]*X[3] + 2*V[8]*T)
  + (1/T)*L*X + (log(T)+1)*M*X);
  annotation(smoothOrder = 10);
end specificGibbsEnergy_TXp_derT_derX;
