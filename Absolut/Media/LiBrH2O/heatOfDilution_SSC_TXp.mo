...
function heatOfDilution_SSC_TXp
  "Differential heat of dilution. Heat that would be released if one unit mass of water were to be added to an infinite amount of solution held at a constant temperature."
  ...
  output SpecificEnthalpy qd "Differential heat of dilution";
...
protected
  ...
  Real[5] c = {0,0,2,3,1.1} "Exponent coef. for A, B and C";
  Real[5] b = {0,0,1,2,0.1} "";
  //Real[5] X = {b[i]*(X_LiBr*100)^c[i] for i in 1:5} "LiBr mass fraction in [%] = {0, 0, X_LiBr^2, 2*X_LiBr^3, 0,1*X_LiBr^1,1}";
  Real[5] X = {0,0,(X_LiBr*100)^2,2*(X_LiBr*100)^3,0.1*(X_LiBr*100)^1.1} "LiBr mass fraction in [%] = {0, 0, X_LiBr^2, 2*X_LiBr^3, 0,1*X_LiBr^1,1}";


algorithm
  qd:= 1000*(A*X - (T^2)*C*X - 2*(T^3)*D*X + pkPa*V[3]*X[3] + (log(T)-1)*L*X - T*M*X);

end heatOfDilution_SSC_TXp;
